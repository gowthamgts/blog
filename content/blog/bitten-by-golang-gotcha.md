...
date = "2023-09-17T00:11:18+05:30"
...
"*visibility timeout*" above refers to the maximum time SQS will wait before sending the same message to another consumer API call **if the message is not deleted** by your consumer. This will be helpful in cases where your consumer pod/instance crashed and the messages should be re-delivered to other consumers. "*maximum receive count*" refers to the maximum number of consumer deliveries SQS will do for a same request before moving it to the [dead-letter](https://en.wikipedia.org/wiki/Dead_letter_queue) queue.
