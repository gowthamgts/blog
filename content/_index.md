Thanks for stopping by. My name is Gowtham Gopalakrishnan and I currently work as 1x systems engineer at [Freshworks](https://freshworks.com). I help manage SMTP services for all Freshworks products. I'm planning to write here regularly about the things I learn along the way.

---

I do occasionally [contribute](https://github.com/gowthamgts) to open-source projects. I'm currently spending some time on writing [DMARC](https://dmarc.org) analysis tool written in golang. I try to [self-host](/blog/self-hosting/) most of the software I use.

---

Apart from work, I love playing RTS and castle-simulation games (lately [Factorio](https://www.factorio.com)). I also regularly play Call of Duty with an online friend, whom I met about 4 years ago.

---

This site is built with [hugo](https://gohugo.io/) and [hugo-bear](https://github.com/janraasch/hugo-bearblog/) theme.

---
